    ...
    if array_length(v_api_result.list_name, 1) = 1 then
        return as_result(
            set_message(
                v_result,
                'Message',
                CONCAT('Group was successfully deleted! (', array_length(v_api_result.list_name, 1), ')'), 'INFO')
            );
    end if;
    if array_length(v_api_result.list_name, 1) > 1 then
        return as_result(
            set_message(
                v_result,
                'Message',
                CONCAT('Groups were successfully deleted! (', array_length(v_api_result.list_name, 1), ')'), 'INFO')
            );
    end if;
